...
################################## New line ############################################
    def handle_detections(self, detection):
        '''Handles detections to publish

        Args:
            ROS detections msg object
        '''

        self._msender.send_message( self._createDetections(detection) )
...
################################## New line ############################################
    def _createDetection(self, detection):
        msg = Json.detection2Json(detection)
        return msg
